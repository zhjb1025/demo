...
function saveUser() {
    ...
    var id=$("#id").val();
    if(id==null || id==""){
        request.service ="eoms_add_user";
        request.loginName =$("#loginName").textbox("getValue");
    }else{
        request.service ="eoms_update_user";
        request.id=id;
    }
...
function queryUser(pageNumber,pageSize) {
    ...
    request.service ="eoms_page_query_user";
    var rsp=ajaxPostSynch(request);
    if(rsp.tradeStatus!=1){
        $.messager.alert('错误提示信息',rsp.rspMsg,'error');
        return ;
...
function updateStatus(index,status) {
    ...
    request.service ="eoms_update_user_status";
    var rsp=ajaxPostSynch(request);
    if(rsp.tradeStatus!=1){
        $.messager.alert('错误提示信息',rsp.rspMsg,'error');
        return ;
...
function resetPassword(index) {
    ...
    request.id=row.id;
    request.status=status;
    request.service ="eoms_reset_password";
    var rsp=ajaxPostSynch(request);
    $.messager.alert('提示信息',rsp.rspMsg,'info');
}
...
function queryRole() {
    var request={};
    request.service ="eoms_query_all_role";
    var rsp=ajaxPostSynch(request);
    if(rsp.tradeStatus!=1){
        $.messager.alert('错误提示信息',rsp.rspMsg,'error');
        return ;
...
function queryBranchInfo() {
    var request={};
    request.service ="eoms_query_all_branch";
    var rsp=ajaxPostSynch(request);
    if(rsp.tradeStatus!=1){
        $.messager.alert('错误提示信息',rsp.rspMsg,'error');
        return ;
