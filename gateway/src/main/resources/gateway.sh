...
##########################################################################################
FUNSERVER_GC_LOG="/root/logs/gateway/gc.log"
FUNSERVER_HOME="/root/app"
FUNSERVER_LOG="/root/logs/gateway/nohup.out"
FUNSERVER_JAR="gateway.jar"
##########################################################################################
...
{
	echo "Stopping ..."
	curl  -X POST http://127.0.0.1:9092/manage/shutdown
	tail -f $FUNSERVER_LOG
}
case "$ACTION" in
    start)
